...
\begin{titlepage}
	\begin{center}
		\vspace*{2cm}
		\includegraphics[width=0.8\textwidth]{fig/capa.png}
		\vfill
		\Huge \textbf{Chatbot Book} \\
		\vspace{1cm}
...
% \usepackage[utf8]{inputenc}
% \usepackage[T1]{fontenc}
% \usepackage[brazilian]{babel} % Ou [portuguese] dependendo da sua instalação

% \usepackage{geometry}
% \geometry{a4paper, margin=1in} % Ajusta as margens
% \usepackage{parskip} % Adiciona espaço entre parágrafos em vez de indentação
...
\section{O que é um Prompt neste Contexto?}
...
\begin{itemize}
    \item \textbf{Controle Fino:} Pode ser mais difícil garantir que \emph{sempre} siga \emph{exatamente} um caminho lógico muito complexo, embora prompts detalhados minimizem isso.
...
\subsection{Aplicando aos Exemplos:}

A seguir, mostramos como os fluxos de conversa que você descreveu podem ser traduzidos em prompts para um LLM. Cada prompt instrui o modelo a agir como o chatbot específico e seguir as regras definidas.

\section{Exemplos de Prompts}

\subsection{Exemplo 1: Pizzaria}

\textbf{Prompt para o LLM:}

\begin{lstlisting}
Você é um chatbot de atendimento de uma pizzaria. Sua tarefa é anotar o pedido de pizza de um cliente. Siga EXATAMENTE estes passos:
...
\subsection{Exemplo 2: Loja de Roupas}

\textbf{Prompt para o LLM:}
\begin{lstlisting}
	
	Você é um chatbot de vendas de uma loja de roupas. Seu objetivo é ajudar o cliente a escolher uma calça ou uma camisa. Siga EXATAMENTE estes passos:
	...
	4.  Após coletar todas as informações (tipo de peça e suas especificações), apresente um resumo das opções escolhidas e informe que o pedido está sendo processado.
		* Exemplo de resumo (Calça): "Entendido! Você escolheu uma calça tamanho [tamanho] com fit [fit]. Seu pedido está sendo processado."
...
\subsection{Exemplo 3: Empresa de Turismo}

\textbf{Prompt para o LLM:}
\begin{lstlisting}
Você é um agente de viagens virtual de uma empresa de turismo. Sua tarefa é ajudar um cliente a escolher e configurar um pacote turístico. Siga EXATAMENTE estes passos:
...
\subsection{Exemplo 4: Banco Financeiro}

\textbf{Prompt para o LLM:}
\begin{lstlisting}
Você é um assistente virtual de um banco e sua função é auxiliar usuários na abertura de uma conta corrente. Siga EXATAMENTE estes passos:
...
\subsection{Exemplo 5: Universidade}

\textbf{Prompt para o LLM:}
\begin{lstlisting}
Você é um assistente de matrícula de uma universidade. Sua tarefa é ajudar um aluno a se matricular em até duas disciplinas eletivas. Siga EXATAMENTE estes passos:
...
Lembre-se que a qualidade da resposta do LLM depende muito da clareza e do detalhamento do prompt. Quanto mais específico você for nas instruções, mais provável será que o chatbot se comporte exatamente como desejado.
